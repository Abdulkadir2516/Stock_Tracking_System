...
namespace Kasa
{
    public partial class Ana_Ekran : Form
    {
        ...
        private void sil_Click(object sender, EventArgs e)
        {
            ...
            if (satislar.SelectedRows.Count > 0)
            {

                foreach (DataGridViewRow row in satislar.SelectedRows)
                {
                    if (row.IsNewRow) continue;

                    string id = satislar.Name.ToString();
                    string barkod = row.Cells[2].Value.ToString();
                    string adet = row.Cells[4].Value.ToString().Replace(",", ".");
                    string saat = row.Cells[1].Value.ToString();

                    baglan.delete("DELETE FROM anlýk_satýs WHERE ýd = '" + id + "' AND barkod = '" + barkod + "' AND adet = '" + adet + "' AND saat = '" + saat + "' LIMIT 1");
                }

                // Tabloyu yeniden yükle
                satislar.DataSource = baglan.fill_table(
                    "SELECT Tarih, Saat, Barkod, Ürün_adý AS Ürün_Adý , Adet, Birim, Satis_Fiyati AS Birim_Fiyatý, Tutar FROM anlýk_satýs WHERE ýd = '" + satislar.Name.ToString() + "' ORDER BY saat DESC",
                    "anlýk_satýs"
                );
            ...
            {
                MessageBox.Show("Lütfen silmek için en az bir satýr seçin.");
            }
        ...
        private void indirimEklemeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            baglan baglan = new baglan();

            DataGridView satislar = islem(tabControl);

            String sayi = (Interaction.InputBox("Oran Deðiþikliði", "Oran Giriniz:"));
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                string barkod = satislar.SelectedRows[0].Cells[2].Value.ToString(); // 2. sütun = Barkod
                barcode.Text = barkod;
            }
            barcode.Focus();
